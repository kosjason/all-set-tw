...
import {
  ...
  getDerivedDepositUntil,
  getNetWorthChartNotes,
...
describe("net worth chart notes", () => {
  const depositRows: NetWorthHistoryRow[] = [
    {
      date: "2026-09-23",
      netWorth: 100,
      assetType: "deposit",
    ...
    { date: "2026-09-25", netWorth: 120, assetType: "deposit", source: "bank" },
  ];

  it("marks the derived deposit range and the deposit-only scope", () => {
    expect(getDerivedDepositUntil(depositRows)).toBe("2026-09-24");
    const points = buildNetWorthChartData(depositRows, ["deposit"], "ALL");
    expect(getNetWorthChartNotes(depositRows, points, ["deposit"])).toEqual([
      "9/25 以前的存款餘額由交易明細推算；無法取得完整明細的帳戶（例如中信）以第一次同步的餘額往前延伸。",
      "目前走勢僅含銀行存款，尚未包含投資部位。",
    ]);
  });

  it("omits the derived note when the visible range starts after it", () => {
    const points = buildNetWorthChartData(
      depositRows,
      ["deposit"],
      "ALL",
  ...
  it("does not claim deposit-only scope when investments are charted", () => {
    const points = buildNetWorthChartData(rows, ["stock", "deposit"], "ALL");
    expect(getNetWorthChartNotes(rows, points, ["stock", "deposit"])).toEqual(
      [],
    );
  });
});
