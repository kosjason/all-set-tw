<script lang="ts">
  ...
  const chartNotes = $derived(
    getNetWorthChartNotes(data, chartData, includedAssets),
  );
  const trendNotice = $derived(
    getNetWorthTrendNotice(chartData.length, comparisonData.length),
  );
  const valueAxis = $derived(
    buildNetWorthValueAxis(
      chartData.flatMap((point) =>
        chartSeries.map(
          (series) => point[series.value as keyof NetWorthChartPoint],
        ),
  ...
  const yTicks = $derived(
    valueAxis
      ? dedupeAxisTicks(valueAxis.ticks, (value) =>
          formatNetWorthAxisValue(value, valueAxis.step),
        )
      : undefined,
  );
  ...
  function formatAxisValue(value: unknown) {
    ...
    return formatNetWorthAxisValue(Number(value), valueAxis?.step ?? 1);
  }
...
<section class="min-w-0 max-w-full" aria-label="資產走勢">
  ...
  <div class="min-w-0 overflow-hidden">
    ...
    {:else}
      ...
      {#if trendNotice}
        <p
          class="flex min-h-24 items-center justify-center rounded-lg bg-ink/2 px-4 text-center text-sm text-subtle"
        >
          {trendNotice}
        </p>
      {:else}
            ...
            {#snippet marks({ context })}
              ...
              {:else}
                {#each selectedSeries as series (series.key)}
                  ...
                  <g
                    opacity={activeFocus && activeFocus !== series.key
                      ? 0.2
                      : 1}
                  >
                    <Spline
      ...
      {#if chartNotes.length}
        <ul
          class="mt-3 grid gap-1 text-caption text-subtle"
          data-testid="net-worth-chart-notes"
        >
          {#each chartNotes as note (note)}
            <li>{note}</li>
          {/each}
        </ul>
      {/if}
