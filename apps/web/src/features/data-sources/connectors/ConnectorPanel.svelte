<script lang="ts">
  ...
  import { getSyncWarning } from "@/data/connectors/sync-status";
      ...
      >
        {error
          ...
          : job?.lastError?.trim()
            ? `上次同步：${job.lastError}`
            : "上次同步失敗，但未取得錯誤原因。"}
      </p>{:else if getSyncWarning(job)}<p class="mt-2 text-sm text-amber-700">
        上次同步成功，但部分資料未取得：{getSyncWarning(job)}
      </p>{/if}
  </div>
