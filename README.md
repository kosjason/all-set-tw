...
更新後資料庫 migration 會自動調整既有資料，不需手動操作：

- **消費分類改為 9 類**：餐飲、交通、居住、購物、3C 數位、娛樂、醫療保險、捐款、其他；收入、轉帳、投資與繳卡費改由「這筆是…」（經濟角色）表示。舊分類的對應：薪資 → 收入（薪資）；轉帳、投資 → 對應的角色；教育 → 其他；娛樂 → 娛樂；手續費、稅務 → 其他；保險、醫療 → 醫療保險；軟體服務 → 3C 數位；生活繳費 → 居住；舊版的捐款 → 捐款、紅包禮金 → 其他。
- **自訂分類保留**：你建立的分類、以及指到它們的個別分類與規則都不會刪除或改指；只有名稱與新系統分類相同時（例如自訂的「捐款」「娛樂」）會改名為「捐款（自訂）」。
- **系統規則的調整保留**：你停用、調整優先序或修改過關鍵字的系統規則會沿用你的設定；被拆分或移除的系統規則，停用狀態會沿用到接替的新規則，改過的關鍵字會轉成你的自訂規則。
